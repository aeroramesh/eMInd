...
class Node(Serializable):
    ...
    def getSocketPosition(self, index: int, position: int, num_out_of: int = 1) -> '(x, y)':
        ...
        if position in (LEFT_BOTTOM, RIGHT_BOTTOM):
            ...
            y = (self.grNode.title_height + self.grNode.title_vertical_padding + self.grNode.edge_roundness + index * self.socket_spacing)-32

        elif position is BOTTOM_CENTER:
            y = self.grNode.height - self.grNode.edge_roundness - self.grNode.title_vertical_padding - index * self.socket_spacing +12
        else:
